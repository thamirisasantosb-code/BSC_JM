...
async function main() {
    // 1. Ler a planilha base com as outras 5 abas
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile('Visao_Executiva_BSC_Master_v3.xlsx');

    // 2. Ler os dados do CSV para montar o novo dashboard
    ...
    // 3. Adicionar a aba do nosso Dashboard
    const sheet = workbook.addWorksheet('6. Dashboard Gerot', { views: [{ showGridLines: false }] });
    ...
    await workbook.xlsx.writeFile('Visao_Executiva_BSC_Completa.xlsx');
    // Update Web Dashboard Data
    const statsWeb = fs.statSync('Relatorio_BSC_15062026.csv');
    const csvContent = fs.readFileSync('Relatorio_BSC_15062026.csv', 'utf8');
    const webDataJs = `
const csvLastModified = '${statsWeb.mtime.toISOString()}';
const rawCsvData = \`${csvContent.replace(/`/g, '\\`')}\`;
`;
    fs.writeFileSync('web_data.js', webDataJs);
    console.log('web_data.js gerado com sucesso para a página Web local!');

    console.log('Visao_Executiva_BSC_Completa.xlsx gerado com sucesso!');
}
